...
STDAPI RegisterFileMenu(HINSTANCE hInst)
{
   //AFX_MANAGE_STATE(AfxGetStaticModuleState( ));
	...
	StringFromIID(CLSID_ShlExtKD, &pwszShellExt);			    
	...
	for(i = 0; ShExClsidEntries[i].hRootKey; i++) 
   {
	   ...
	   if(NOERROR == lResult) 
      {
			...
			int szDataLen = (lstrlen(szData) + 1) * sizeof(TCHAR);
	...
	REGSTRUCT OtherShExEntries[] = {  
			...
			HKEY_CLASSES_ROOT,	TEXT("*\\shellex\\ContextMenuHandlers\\"_T(SHELLEXNAME)), NULL,		TEXT("%s"),
...
/**************************************************************************
**************************************************************************/
STDAPI UnregisterFileMenu(HINSTANCE hInst)
{
	...
	REGSTRUCT OtherShExEntries[] = {  
			...
			// our CLSID is placed in the "%s", and so, explorer will take that CLSID, look in HKEY_CLASSES_ROOT/CLSID
			// to find a matching CLSID and use the info in there to find our DLL... whew. 
			HKEY_CLASSES_ROOT,	TEXT("*\\shellex\\ContextMenuHandlers\\"_T(SHELLEXNAME)), NULL,		NULL,
	...
	// NT needs to have shell extensions "approved".
	if (VER_PLATFORM_WIN32_NT == osvi.dwPlatformId)  {
		lstrcpy( szSubKey, TEXT("Software\\Microsoft\\Windows\\CurrentVersion\\Shell Extensions\\Approved\\"_T(SHELLEXNAME)));
...
STDAPI DllRegisterServer(void)
{
	...
	HRESULT h = RegisterFileMenu(g_hModule);
...
STDAPI DllUnregisterServer(void)
{
	...
	HRESULT h = UnregisterFileMenu(g_hModule);
